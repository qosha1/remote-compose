...
def list_instances(region: str = None):
    ...
    try:
        ...
        print(f"{'Instance ID':<22} {'Name':<30} {'Type':<12} {'Public IP':<16} {'State'}")
        print("-" * 95)

        for inst in instances:
            name = inst.get('name', '-')[:28]
            print(f"{inst['instance_id']:<22} {name:<30} {inst.get('instance_type', '-'):<12} {inst.get('public_ip', '-'):<16} {inst.get('state', '-')}")
...
def create_or_get_target(
    ...
    from remote_compose.services import AWSService, TargetService
    ...
    if instance_name:
        ...
        raise ValueError("Must provide either --instance-id, --instance-name, or --target")

    print(f"Found instance: {instance['instance_id']} ({instance.get('name', 'unnamed')})")
...
Examples:
    ...
    parser.add_argument(
        '--instance-id',
        help='EC2 instance ID to deploy to'
    )
    parser.add_argument(
        '--instance-name',
        help='EC2 instance Name tag to deploy to'
    )
    parser.add_argument(
        '--target',
        help='Existing target name to deploy to'
    )
    parser.add_argument(
        '--ssh-key',
        dest='ssh_key_path',
