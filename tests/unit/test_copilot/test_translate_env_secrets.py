...
class TestVariables:
    def test_simple_variables_become_compose_environment(self):
        compose_env, rc_secrets, _ = translate_env_and_secrets(
            _svc(
                {
                    "name": "api",
                    "variables": {
    ...
    def test_int_and_bool_variables_stringified(self):
        ...
        compose_env, _, _ = translate_env_and_secrets(
            _svc(
                {
                    "name": "api",
...
class TestSecrets:
    def test_secretsmanager_pointer_becomes_aws_sm_secret(self):
        _, rc_secrets, _ = translate_env_and_secrets(
            _svc(
                {
                    "name": "api",
                    "secrets": {
        ...
        assert (
            s["arn"]
            == "arn:aws:secretsmanager:us-west-2:123456789012:secret:prod/db-AbCdEf"
        )
