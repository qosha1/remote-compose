...
def build_deploy_context(
    ...
    *,
    ...
    require_resolvable_images: Optional[bool] = None,
) -> DeployContext:
    ...
    ``require_resolvable_images`` answers a different question — not "may
    compose be absent?" but "will this stack try to *run* a container?" — and
    rejects a deploy set containing a service with no image anywhere and no
    build context (rc-2r1r). It defaults to following ``require_compose_file``
    because the two are genuinely dependent, not merely correlated: compose is
    where rc reads images from, so with the file absent a compose-only
    service's image is unknowable and demanding one would be incoherent. Pass
    it explicitly to separate them.
    """
    if require_resolvable_images is None:
        require_resolvable_images = require_compose_file
    ...
    # rc-2r1r: every service in the deploy set must resolve to an image rc can
    # pull, or a build context rc can build. Neither means services.tf.j2
    # falls through to "${aws_ecr_repository.<svc>.repository_url}:latest" and
    # the stack applies clean against an ECR repo rc never pushes to — the
    # same silent failure startsim-wxb7 closed for a missing compose file,
    # reached here by rc.yml declaring a service compose doesn't have.
    # Checked against the post-filter set: an excluded service isn't deployed,
    # so its image doesn't have to exist.
    if require_resolvable_images:
        from .config._schema_types import ConfigError as _ConfigError

        unresolvable = sorted(
            name
            for name in deploy_names
            if not (v2.services.get(name) and v2.services[name].image)
            and not (compose_services.get(name) or {}).get("image")
            and not (compose_services.get(name) or {}).get("build")
        )
        if unresolvable:
            raise _ConfigError(
                f"service(s) {unresolvable} have no image to run: not defined "
                f"in {compose_path} (which is where rc reads `image:` and "
                f"`build:` from) and no `services.<name>.image` in "
                f"{rc_yml_path.name}. Add the service to compose, or declare "
                f"the pre-built image in rc.yml."
            )
    ...
    for name in sorted(deploy_names):
        ...
        if svc is not None:
            ...
            # rc-2r1r: rc.yml services.<svc>.image overrides compose, on the
            # same principle as dockerfile above — the rc.yml layer wins so
            # users can point rc at a pre-built image without editing their
            # compose file. Dropping the build context is what makes the
            # override real: services.tf.j2 tests has_build_context FIRST, so
            # leaving it set would have rc build and push to ECR while the
            # image the user declared went silently unused.
            image_override = img
            if svc.image:
                image_override = svc.image
                bc, bargs, target = None, {}, None
