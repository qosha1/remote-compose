...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--service-config", help="Path to service configuration YAML file"
        )
        parser.add_argument(
            "--secrets-file",
            action="append",
        ...
        parser.add_argument("--certificate-arn", help="ACM certificate ARN for HTTPS")
        parser.add_argument(
            "--vpc-cidr",
            default="10.0.0.0/16",
        ...
        parser.add_argument(
            "--project-name", "-p", help="Project name (default: directory name)"
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if result.success:
            self.stdout.write("")
            self.stdout.write(
                self.style.SUCCESS("Infrastructure provisioning complete!")
            )
            ...
            if context.warnings:
                self.stdout.write("")
                self.stdout.write(
                    self.style.WARNING(f"Warnings ({len(context.warnings)}):")
                )
