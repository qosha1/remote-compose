...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--name", required=True, help="Unique name for the target")
        parser.add_argument(
            "--host", required=True, help="Remote host address (IP or hostname)"
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--description", default="", help="Description of the target"
        )
        parser.add_argument(
            "--aws-instance-id",
        ...
        parser.add_argument(
            "--aws-region", dest="aws_region", help="AWS region (optional)"
        )
        parser.add_argument(
            "--no-validate", action="store_true", help="Skip SSH connection validation"
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if options["no_validate"]:
                self.stdout.write(
                    self.style.WARNING(
                        "Connection was not validated. Run 'python manage.py test_target --name {}' to verify.".format(
                            target.name
                        )
                    )
                )
