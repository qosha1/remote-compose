...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        list_parser.add_argument('--aws', action='store_true', help='List clusters from AWS')
        ...
        # Import cluster
        import_parser = subparsers.add_parser('import', help='Import existing AWS cluster')
        ...
        delete_parser.add_argument('--delete-aws', action='store_true', help='Also delete in AWS')
    ...
    def handle_list(self, ecs_service, options):
        if options.get('aws'):
            ...
            self.stdout.write(f"{'Name':<30} {'Status':<15} {'Services':<10} {'Tasks':<10}")
            ...
            self.stdout.write(f"{'Name':<25} {'AWS Name':<25} {'Region':<15} {'Status':<12} {'Type':<10}")
    ...
    def handle_show(self, ecs_service, options):
        ...
        self.stdout.write(f"  Security Groups: {cluster.security_group_ids or 'Not configured'}")
        ...
        if services:
            ...
            for service in services:
                self.stdout.write(f"    - {service.name}: {service.running_count}/{service.desired_count} running")
    ...
    def handle_delete(self, ecs_service, options):
        ...
        if not options.get('force'):
            ...
            if options.get('delete_aws'):
                self.stdout.write(self.style.WARNING("This will also delete the cluster in AWS!"))
