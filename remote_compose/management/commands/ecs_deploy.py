...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--cluster',
            required=True,
            help='ECS cluster name to deploy to'
        )
        parser.add_argument(
            '--compose-file',
            '-f',
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if deployment.metadata.get('service_arn'):
                self.stdout.write(f"  Service ARN: {deployment.metadata['service_arn']}")
            if deployment.metadata.get('task_definition_arn'):
                self.stdout.write(f"  Task Definition: {deployment.metadata['task_definition_arn']}")
