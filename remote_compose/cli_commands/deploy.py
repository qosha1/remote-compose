...
@click.option(
    "--code-only", is_flag=True, help="Deploy only code services (skip infrastructure)"
)
@click.option(
    "--services",
    "selected_services",
    default=None,
    help="Comma-separated services to deploy",
)
@click.option(
    "--ttl",
    "ttl",
...
def deploy_cmd(
    ...
    try:
        ...
        click.echo(
            f"Error: Cluster '{config['cluster']}' not found. Run 'rc provision' first.",
            err=True,
        )
        sys.exit(1)
    ...
    if result.success:
        ...
        click.echo(
            f"  Deployment failed at '{result.failed_step}': {result.error}", err=True
        )
        sys.exit(1)
