...
class BuildRecord(TimestampedModel):
    ...
    service_name = models.CharField(
        max_length=255,
        db_index=True,
        help_text="Docker Compose service name"
    )
    ...
    context_path = models.CharField(
        max_length=1024,
        blank=True,
        help_text="Docker build context path"
    )
    dockerfile_path = models.CharField(
        max_length=1024,
        blank=True,
        help_text="Dockerfile path relative to context"
    )
    ...
    build_started_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text="When the Docker build started"
    )
    build_completed_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text="When the Docker build completed"
    )
    push_started_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text="When ECR push started"
    )
    push_completed_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text="When ECR push completed"
    )
    ...
    build_log = models.TextField(
        blank=True,
        help_text="Docker build output log"
    )
    image_digest = models.CharField(
        max_length=128,
        blank=True,
        help_text="Image digest from ECR after push"
    )
    ...
    build_args = models.JSONField(
        default=dict,
        blank=True,
        help_text="Docker build arguments"
    )
    platform = models.CharField(
        max_length=50,
        default='linux/amd64',
        help_text="Target platform for the build"
    )

    # Cache information
    cache_hit = models.BooleanField(
        default=False,
        help_text="Whether this build used cached layers"
    )
    ...
    metadata = models.JSONField(
        default=dict,
        blank=True,
        help_text="Additional build metadata"
    )
    ...
    def complete_build(self):
        ...
        self.save(update_fields=['status', 'build_completed_at', 'push_started_at', 'updated_at'])

    def succeed(self, image_uri: str, image_digest: str = '', image_size_bytes: int = None):
        ...
        self.save(update_fields=[
            'status', 'push_completed_at', 'image_uri',
            'image_digest', 'image_size_bytes', 'updated_at'
        ])
    ...
    def fail(self, error_message: str, build_log: str = ''):
        ...
        if not self.build_completed_at:
            self.build_completed_at = timezone.now()
        self.save(update_fields=[
            'status', 'error_message', 'build_log',
            'build_completed_at', 'updated_at'
        ])
    ...
    def mark_skipped(self, image_uri: str, reason: str = 'Using cached image'):
        ...
        self.save(update_fields=['status', 'image_uri', 'cache_hit', 'metadata', 'updated_at'])
...
class DeploymentEvent(TimestampedModel):
    ...
    previous_state = models.CharField(
        max_length=50,
        blank=True,
        help_text="Previous state before this event"
    )
    new_state = models.CharField(
        max_length=50,
        blank=True,
        help_text="New state after this event"
    )
    ...
    resource_id = models.CharField(
        max_length=512,
        blank=True,
        help_text="AWS resource ID or ARN"
    )
    ...
    error_code = models.CharField(
        max_length=100,
        blank=True,
        help_text="Error code if this is an error event"
    )
    stack_trace = models.TextField(
        blank=True,
        help_text="Stack trace for error events"
    )
...
class ResourceMetric(TimestampedModel):
    ...
    unit = models.CharField(
        max_length=20,
        blank=True,
        help_text="Unit of measurement (%, bytes, ms, count)"
    )
    ...
    period_seconds = models.IntegerField(
        default=60,
        help_text="Duration of the measurement period in seconds"
    )

    # Sample information
    sample_count = models.IntegerField(
        default=1,
        help_text="Number of data points in this aggregation"
    )
    ...
    @classmethod
    def record_ecs_metric(cls, ecs_service, metric_type, value, period_start, period_end, **kwargs):
        """Record a metric for an ECS service."""
        return cls._record_metric('ecs_service', ecs_service, metric_type, value, period_start, period_end, **kwargs)

    @classmethod
    def record_efs_metric(cls, efs_file_system, metric_type, value, period_start, period_end, **kwargs):
        """Record a metric for an EFS file system."""
        return cls._record_metric('efs_file_system', efs_file_system, metric_type, value, period_start, period_end, **kwargs)

    @classmethod
    def record_cluster_metric(cls, cluster, metric_type, value, period_start, period_end, **kwargs):
        """Record a metric for an ECS cluster."""
        return cls._record_metric('cluster', cluster, metric_type, value, period_start, period_end, **kwargs)
