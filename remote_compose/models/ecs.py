...
class ECSCluster(TimestampedModel):
    ...
    aws_cluster_name = models.CharField(
        max_length=255, help_text="Cluster name in AWS ECS"
    )
    ...
    is_managed = models.BooleanField(
        default=True, help_text="Whether this cluster is managed by remote-compose"
    )

    # Networking (for Fargate)
    vpc_id = models.CharField(
        max_length=50, null=True, blank=True, help_text="VPC ID for Fargate tasks"
    )
    subnet_ids = models.JSONField(
        default=list, blank=True, help_text="List of subnet IDs for task placement"
    )
    security_group_ids = models.JSONField(
        default=list, blank=True, help_text="List of security group IDs for tasks"
    )

    # Status
    status = models.CharField(
        max_length=20, choices=ClusterStatus.choices, default=ClusterStatus.PENDING
    )
...
class ECSTaskDefinition(TimestampedModel):
    ...
    # Identification
    name = models.CharField(
        max_length=255, db_index=True, help_text="Task definition family name"
    )
    ...
    revision = models.IntegerField(
        default=1, help_text="Task definition revision number"
    )

    # Source
    source_compose_file = models.TextField(
        blank=True, help_text="Original docker-compose.yml content"
    )
    ...
    container_definitions = models.JSONField(
        default=list, help_text="ECS container definitions (converted from compose)"
    )
    ...
    memory = models.CharField(
        max_length=20, default="512", help_text="Memory in MB (minimum 512 for Fargate)"
    )

    # Compatibility
    requires_compatibilities = models.JSONField(
        default=list, help_text="Required compatibility types (FARGATE, EC2)"
    )
    ...
    volumes = models.JSONField(
        default=list, blank=True, help_text="ECS volume definitions"
    )
...
class ECSService(TimestampedModel):
    ...
    # Identification
    name = models.CharField(max_length=255, db_index=True, help_text="Service name")
    cluster = models.ForeignKey(
        ECSCluster, on_delete=models.CASCADE, related_name="services"
    )
    ...
    desired_count = models.IntegerField(
        default=1, help_text="Desired number of running tasks"
    )
    min_count = models.IntegerField(
        default=1, help_text="Minimum number of tasks (for auto-scaling)"
    )
    max_count = models.IntegerField(
        default=10, help_text="Maximum number of tasks (for auto-scaling)"
    )

    # Running state (synced from AWS)
    running_count = models.IntegerField(
        default=0, help_text="Current number of running tasks"
    )
    pending_count = models.IntegerField(
        default=0, help_text="Current number of pending tasks"
    )
    ...
    deployment_minimum_percent = models.IntegerField(
        default=50, help_text="Minimum healthy percent during deployment"
    )
    deployment_maximum_percent = models.IntegerField(
        default=200, help_text="Maximum percent during deployment"
    )
    ...
    container_port_for_lb = models.IntegerField(
        null=True, blank=True, help_text="Container port for load balancer"
    )
    ...
    last_deployment_at = models.DateTimeField(
        null=True, blank=True, help_text="Last successful deployment time"
    )
    ...
    service_connect_enabled = models.BooleanField(
        default=False, help_text="Whether Service Connect is enabled for this service"
    )
    service_connect_namespace = models.CharField(
        max_length=255, blank=True, help_text="Service Connect namespace name"
    )
    service_connect_port_name = models.CharField(
        max_length=255, blank=True, help_text="Port name for Service Connect discovery"
    )
    ...
    def is_healthy(self):
        ...
        return (
            self.status == self.ServiceStatus.ACTIVE
    ...
    def to_aws_create_format(self) -> dict:
        ...
        service_def = {
            ...
            "taskDefinition": self.task_definition.aws_task_definition_arn
            or self.task_definition.full_arn,
            "desiredCount": self.desired_count,
            "launchType": self.cluster.launch_type.upper(),
            "deploymentController": {"type": self.deployment_controller},
        ...
        if self.cluster.launch_type == ECSCluster.LaunchType.FARGATE:
            ...
            service_def["networkConfiguration"] = {
                "awsvpcConfiguration": {
                    ...
                    "assignPublicIp": ("ENABLED" if cluster_public else "DISABLED"),
                }
            }
...
class ECRRepository(TimestampedModel):
    ...
    # Identification
    name = models.CharField(
        max_length=255, db_index=True, help_text="Repository name in ECR"
    )

    # AWS Details
    aws_repository_arn = models.CharField(
        max_length=512, unique=True, db_index=True, help_text="AWS ECR Repository ARN"
    )
    aws_repository_uri = models.CharField(
        max_length=512, help_text="Full URI for pushing/pulling images"
    )
    aws_region = models.CharField(
        max_length=50, help_text="AWS region where repository is located"
    )
    ...
    # Management
    is_managed = models.BooleanField(
        default=True, help_text="Whether this repository was created by remote-compose"
    )
    ...
    def update_from_aws(self, aws_response: dict) -> None:
        ...
        """
        self.aws_repository_arn = aws_response.get(
            "repositoryArn", self.aws_repository_arn
        )
        self.aws_repository_uri = aws_response.get(
            "repositoryUri", self.aws_repository_uri
        )
        ...
        self.metadata.update(
            {
                ...
                "image_scanning_configuration": aws_response.get(
...
class EFSFileSystem(TimestampedModel):
    ...
    # Identification
    name = models.CharField(
        max_length=255, db_index=True, help_text="Friendly name for this file system"
    )
    ...
    aws_region = models.CharField(
        max_length=50, help_text="AWS region where file system is located"
    )
    ...
    # Networking
    vpc_id = models.CharField(
        max_length=50, help_text="VPC ID where the file system is deployed"
    )
    ...
    # Management
    is_managed = models.BooleanField(
        default=True, help_text="Whether this file system was created by remote-compose"
    )
    ...
    def update_from_aws(self, aws_response: dict) -> None:
        ...
        """
        self.aws_file_system_id = aws_response.get(
        ...
        self.save(
            update_fields=["aws_file_system_id", "name", "metadata", "updated_at"]
        )
