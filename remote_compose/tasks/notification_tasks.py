...
def _validate_webhook_url(url: str) -> bool:
    ...
    try:
        ...
        if not allow_all:
            allowed_domains = get_setting(
                "WEBHOOK_ALLOWED_DOMAINS", ALLOWED_WEBHOOK_DOMAINS
            )
...
def send_deployment_notification(
    ...
    """
    try:
        deployment = Deployment.objects.select_related("target", "context").get(
            id=deployment_id
        )
...
def send_email_notification(
    ...
    status = deployment.status
    subject = (
        f"[remote-compose] {deployment.project_name} - {event.split('.')[-1].title()}"
    )
